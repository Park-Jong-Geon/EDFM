...
class FlaxResNetforFED(nn.Module):
    depth:        int = 20
    ...
    @nn.compact
    def __call__(self, x, z, noise_key, **kwargs):
        ...
        noise_scale = nn.Dense(features=len(num_blocks)+1,
                               use_bias=False,
                               kernel_init=jax.nn.initializers.constant(1.),
                               dtype=self.dtype,)(jnp.ones((1, 1)))

        # define intermediate layers...
        for layer_idx, num_block in enumerate(num_blocks):
            # Functional Ensemble Distillation
            noise_key, _ = jax.random.split(noise_key)
            y += noise_scale[0][layer_idx] * self.noise_std * jax.random.normal(key=noise_key, shape=y.shape)
        ...
        # Functional Ensemble Distillation
        noise_key, _ = jax.random.split(noise_key)
        y += noise_scale[0][layer_idx+1] * self.noise_std * jax.random.normal(key=noise_key, shape=y.shape)
