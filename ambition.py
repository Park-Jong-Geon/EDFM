...
if __name__ == '__main__':
    logit_2steps = np.load('logits/CIFAR100_x32_fm_2steps.npy')
    logit_100steps = np.load('logits/CIFAR100_x32_fm_200steps.npy')
    labels = np.load('data/CIFAR100_x32/test_labels.npy')

    for logits, name in zip([logit_2steps, logit_100steps], ['2steps', '200steps']):
        acc = nll = 0
        prob = softmax(logits, axis=-1)
        for i in range(30):
            acc += compute_acc(prob[:, i], labels)
            nll += compute_nll(prob[:, i], labels)
        acc /= 30
        nll /= 30
        ens_acc = compute_acc(softmax(logits, axis=-1).mean(1), labels)
        ens_nll = compute_nll(softmax(logits, axis=-1).mean(1), labels)
        ece = compute_ece(softmax(logits, axis=-1).mean(1), labels)
        amb = compute_amb(logits, labels)
        var = compute_var(logits, labels)
        print(f'{name}: acc={acc:.4f}, nll={nll:.4f}, ens_acc={ens_acc:.4f}, ens_nll={ens_nll:.4f}, ece={ece:.4f}, amb={amb}, var={var}')