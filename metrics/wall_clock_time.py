...
def launch(config, print_fn):
    ...
    def step_label(state, batch):
        ...
        for swag_param in swag_param_list:
            ...
            # logits0 = logits0 / logits0.std(-1, keepdims=True)
    ...
    def loss_func(params, state, batch, train=True):
        ...
        epsilon, u_t, _, x_t, t = output[0] if train else output
        
        # Standardized logit
        logitsA = logitsA - logitsA.mean(-1, keepdims=True)
        logitsA = logitsA / logitsA.std(-1, keepdims=True)
        
        score_loss = mse_loss(epsilon, u_t)
        for T in [1,2,3,4,5]:
            p_ens = jax.nn.softmax(logitsA / T)
            score_loss += config.ce_lambda * (1-t)**2 * T**2 * ce_loss_with_target(x_t / T, p_ens) / 5
    ...
    def sample_func_2(state, batch, steps):
        return None
        # labels = batch["labels"]
        # drop_rng, score_rng = jax.random.split(state.rng)
    ...
    @partial(jax.pmap, axis_name="batch")
    def step_sample_2(state, batch):
        steps = config.T
        metrics = sample_func_2(state, batch, steps=steps)
        # metrics = jax.lax.psum(metrics, axis_name="batch")
        return metrics
    ...
    input = jnp.ones((32, 32, 32, 3))

    params_dict = pdict(
        params=state.ema_params,
        image_stats=config.image_stats,
        batch_stats=state.batch_stats)
    rngs_dict = dict(dropout=jax.random.PRNGKey(42))
    model_bd = dbn.bind(params_dict, rngs=rngs_dict)
    ...
    print(f"Elapsed time with score: {end-start:.8f} sec")

    start = time.time()
    prob_ens, _ = model_bd.sample_2(
        jax.random.PRNGKey(42), None, input, config.num_ensembles)
    end = time.time()
    ...
    #         start2 = time.time()
    #         acc_metrics = step_sample_2(state, batch)
    #         end2 = time.time()
            
    #         print(f"Elapsed time without score usage: {end2-start2:.8f} sec")
    #         print(f"Elapsed time with score usage: {end-start:.8f} sec")
