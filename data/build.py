...
def build_dataloaders(config, corrupted=False):
    ...
    if config.data_name == 'cinic10_nocifar':
        if config.data_subname == 'c10':
            val_images = np.load(os.path.join(
                config.data_root, f'{config.data_name}/valid_images.npy'))
            c10_trn_images = np.load(os.path.join(
                config.data_root, f'CIFAR10_x32/train_images.npy'))
            c10_val_images = c10_trn_images[40960:]
            cinic_images = np.concatenate([trn_images, val_images, tst_images], axis=0)
            
            trn_images = np.concatenate([c10_val_images, cinic_images[0:40960-9040]], axis=0)
            ...
            tst_images = np.load(os.path.join(
                config.data_root, f'CIFAR10_x32/test_images.npy'))
            tst_labels = np.load(os.path.join(
                config.data_root, f'CIFAR10_x32/test_labels.npy'))

            image_shape = (1, 32, 32, 3)
            num_classes = 10

        elif config.data_subname == 'c100':
            val_images = np.load(os.path.join(
                config.data_root, f'{config.data_name}/valid_images.npy'))
